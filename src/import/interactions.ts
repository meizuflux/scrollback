...
const processInteractionFile = async (
    ...
    for (let i = 0; i < items.length; i++) {
        ...
        if (i % Math.max(1, Math.floor(items.length / 10)) === 0 || i === items.length -1) { // Update ~10 times
...
export const importPostLikes = async (files: File[], database: InstagramDatabase, onProgress?: (progress: number, statusText?: string) => void) => {
...
export const importComments = async (files: File[], database: InstagramDatabase, onProgress?: (progress: number, statusText?: string) => void) => {
    ...
    for (let i = 0; i < commentsFile.length; i++) {
        ...
        if (i % Math.max(1, Math.floor(commentsFile.length / 10)) === 0 || i === commentsFile.length -1) { // Update ~10 times
...
export const importSavedPosts = async (files: File[], database: InstagramDatabase, onProgress?: (progress: number, statusText?: string) => void) => {
    await processInteractionFile(
