    ...
    return (
        <div>
            <div class="p-4 bg-white rounded-lg shadow mb-4">
                ...
                <p>Messages Liked: {messages.filter(m => m.reactions?.some(r => r.actor === data.user.name && r.reaction === '❤')).length}</p>
                <p>Reels Sent: {messages.filter(m => m.sender_name === data.user.name && m.share?.link?.includes('/reel/')).length}</p>
                <p>Reels Received: {messages.filter(m => m.sender_name !== data.user.name && m.share?.link?.includes('/reel/')).length}</p>
