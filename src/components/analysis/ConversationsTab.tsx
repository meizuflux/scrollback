	...
	<section class="space-y-5">
		<div>
			<div class="flex flex-col justify-between gap-3 md:flex-row md:items-end">
				<div>
					<h1 class="font-sans text-3xl font-semibold tracking-tight text-white">Conversations</h1>
		...
		<div class="rounded-lg border border-purple/30 bg-gray-900/85 p-4">
		...
		>
			...
			>
				<div class="overflow-hidden rounded-lg border border-purple/35 bg-gray-900/90">
									<div class="hidden grid-cols-[minmax(0,1fr)_120px_140px_130px] gap-4 border-b border-gray-700 bg-purple/10 px-5 py-3 text-xs font-semibold uppercase tracking-wider text-gray-500 sm:grid">
						...
						<For each={props.filteredConversations}>
							{(conversation) => (
								<>
									<button
										type="button"
					class="grid w-full cursor-pointer grid-cols-[minmax(0,1fr)_auto] gap-3 border-b border-gray-600/45 bg-gray-800/60 px-5 py-4 text-left transition-colors hover:border-purple/60 hover:bg-purple/10 focus-visible:outline-2 focus-visible:outline-inset focus-visible:outline-purple sm:grid-cols-[minmax(0,1fr)_120px_140px_130px_auto] sm:items-center sm:gap-4"
						...
						<span class="text-sm font-semibold text-pink">
									...
									<Show when={props.selectedConversation() === conversation.title}>
										<div class="border-t border-gray-700 bg-gray-900 px-5 py-4">
											<h2 class="mb-3 font-sans text-sm font-semibold text-white">Messages by person</h2>
											<Show
												when={!props.conversationStatsLoading()}
												fallback={<p class="text-sm text-gray-400">Loading stats…</p>}
											>
												<Show
													when={props.conversationStats().length > 0}
												...
												>
													<div class="max-h-[50vh] overflow-y-auto rounded-md border border-gray-700">
														<table class="w-full text-sm">
															<thead class="sticky top-0 bg-gray-800 text-left text-xs uppercase tracking-wider text-gray-500">
																<tr>
															...
															<tbody class="divide-y divide-gray-700">
																<For each={props.conversationStats()}>
																	{(stat) => (
																		<tr>
																			<td class="px-4 py-2.5 text-gray-100">
																				{stat.sender}
																			</td>
																			<td class="px-4 py-2.5 text-right font-semibold text-purple">
